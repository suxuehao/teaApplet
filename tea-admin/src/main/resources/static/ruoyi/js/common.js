...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        if (isScrollToTop) {
            ...
            $('.mainContent', topWindow).find('iframe.RuoYi_iframe').css({
                "visibility": "hidden",
                "position": "absolute"
...
function openToCurrentTab(obj) {
    if (isScrollToTop) {
        ...
        $(obj).css({
            "visibility": "visible",
            "position": "static"
        }).siblings('.RuoYi_iframe').css({"visibility": "hidden", "position": "absolute"});
    }
}
 ...
 * @param height 图片显示宽度
 */
function viewImgFilePath(filePath, width, height){
    top.layer.open({
        title: false,
        type: 1,
        closeBtn: true,
        shadeClose: true,
        area: ['auto', 'auto'],
        content: "<img src='" + filePath + "' height='" + height + "' width='" + width + "'/>"
    });
}
